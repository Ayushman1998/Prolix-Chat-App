...
let User = class {
    ...
    constructor({ username, name, email, password, profilePic }) {
        this.name = name;
    ...
    static findUser({searcher, keyword, offset}) {
        return new Promise(async (resolve, reject) => {
            let dbUser

            try {
                if (validator.isEmail(keyword)) {
                    dbUser = await UserSchema.aggregate([
                        {
                            $match : { email : keyword , _id : { $ne : searcher } }
                        },
                        { $facet: {
                            data: [
                    ...
                    dbUser = await UserSchema.aggregate([
                        {
                            $match : { "name" : {$regex : keyword, $options : 'i'} , _id : { $ne : searcher } }
                        },
                        { $facet: {
                            data: [
                                {"$skip": parseInt(offset)},
